...
### US-186 LLM 出力トークン切断時の JSON 修復とプロンプト短縮（P1）【完了】
...
### US-189 uvicorn ファイルウォッチの監視範囲最適化（P1）【完了】

開発者として、uvicorn の `--reload` による CPU 消費を最小限に抑えたい。
なぜなら、現在 `--reload` が `/workspace` 全体をポーリングしており、`node_modules/`・`.git/`・`frontend/dist/` 等の不要なディレクトリまで監視して CPU 5% を常時消費するから。

- 受け入れ基準
  - Given DevContainer 内で uvicorn が起動しているとき、When `ps aux` で CPU 使用率を確認する、Then uvicorn の CPU 使用率が従来（約 5%）から 1% 以下に低減している … **OK**: `--reload-dir app` により監視範囲を app/ に限定
  - Given uvicorn が `--reload` モードで起動しているとき、When `app/` ディレクトリ内の .py ファイルを変更する、Then 従来通りホットリロードが動作する … **OK**: app/ を監視対象にしているため従来通り動作
  - Given `frontend/`、`node_modules/`、`.git/` 内のファイルを変更したとき、When uvicorn の動作を確認する、Then 不要なリロードが発生しない … **OK**: 監視対象外のためリロードされない

### US-190 ヘルスチェック間隔の適正化（P2）【完了】

開発者として、ServiceStatusPanel と docker-compose のヘルスチェック間隔を適正化したい。
なぜなら、ServiceStatusPanel は 15 分間隔で鮮度が低い一方、docker-compose の Ollama ヘルスチェックは 10 秒間隔で過剰であり、どちらもバランスが取れていないから。

- 受け入れ基準
  - Given ServiceStatusPanel が表示されているとき、When サービスの状態が変わる、Then 5 分以内にパネルの表示が更新される（現状 15 分 → 5 分に短縮） … **OK**: setInterval を 5 * 60 * 1000 に変更
  - Given docker-compose.yml の Ollama ヘルスチェック、When 設定を確認する、Then interval が 5m に変更されている（現状 10s → 5m に緩和） … **OK**: interval を 5m に変更
