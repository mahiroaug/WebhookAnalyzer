...
#### US-176 INBOX ヘッダーの簡素化と URL 全文表示（P1）【実装中】
...
- 受け入れ基準
  - Given INBOX 一覧ペインが表示されている、When ヘッダー領域を確認する、Then Webhook 受信 URL 表示バー（コピーボタン付き）が表示されていない
  - Given サービス接続状況パネルが表示されている、When テーブルを確認する、Then "Label" / "URL" / "Status" のカラムヘッダー行が表示されていない
  - Given サービス接続状況パネルが表示されている、When 各行の URL を確認する、Then パネル幅に収まる範囲で URL が全文表示されている（幅が不足する場合のみ末尾省略される）
