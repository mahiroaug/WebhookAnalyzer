...
#### US-003 ページングで大量データを安定閲覧（P0）【未着手】
...
#### US-004 状態別UI（読み込み/空/エラー）の明確化（P1）【未着手】
...
#### US-006 Payload JSONの要素分解表示（P0）【未着手】
...
#### US-012 比較ビュー（同一group_key/イベント比較）（P1）【未着手】
...
#### US-014 event_type別の網羅一覧（P0）【未着手】
...
#### US-015 ペイロード構造のスキーマ自動推定（P0）【未着手】
